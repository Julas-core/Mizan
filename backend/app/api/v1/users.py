...
def get_user_summary(user_id: str, db: Session = Depends(dependencies.get_db), current_user = Depends(get_current_user)):
    ...
    # Update S2S via the new intelligence layer
    from app.services.safe_to_spend import calculate_safe_to_spend
    safe_to_spend_cents, days_to_next_bill, next_bill_amount_cents, upcoming_bills = calculate_safe_to_spend(user)
    ...
    return UserSummary(
        ...
        days_to_next_bill=days_to_next_bill,
        next_bill_amount_cents=next_bill_amount_cents,
