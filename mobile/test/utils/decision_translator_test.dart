...
import '../../lib/utils/decision_translator.dart';
...
void main() {
  group('DecisionTranslator', () {
    test('Translates severe risk / deficit mode correctly', () {
      ...
      expect(result.primaryReason, 'This will leave you short before your next income.');
    });

    test('High behavior penalty with moderate affordability results in Avoid', () {
      final input = {
      ...
      expect(result.tinyStat, isNull); // Behavior doesn't have a tiny stat by design
    });
    ...
    test('Prioritizes Behavior > Affordability > Goal in tie-breaking', () {
      ...
      final result = DecisionTranslator.translate(input);
      expect(result.verdict, 'Safe to buy'); // Since behavior < 0.15 threshold theoretically, but risk exists
      expect(result.primaryReason, 'You tend to regret purchases like this.');
    });
