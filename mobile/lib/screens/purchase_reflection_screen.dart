...
class _PurchaseReflectionScreenState extends State<PurchaseReflectionScreen> {
  ...
  void _submitReflection(int option) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Reflection saved. AI is calibrating...')),
    );
    
    // Auto navigate back after a small delay
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Purchase Reflection', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
        ...
        child: Column(
          children: [
                ...
                children: [
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('RECENT PURCHASE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.white54, letterSpacing: 1)),
                        SizedBox(height: 4),
                        Text('Premium Coffee Maker', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                        SizedBox(height: 2),
                        Text('Kitchen & Home • \$189.00', style: TextStyle(fontSize: 12, color: Colors.white54)),
                      ],
                    ),
                  )
                ],
            ...
            Container(
              ...
              child: const Icon(Icons.psychology_alt, color: primaryColor, size: 60),
            ),
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () => _submitReflection(0),
                    child: Container(
                      padding: const EdgeInsets.all(24),
                      decoration: BoxDecoration(
                        color: _selectedOption == 0 ? Colors.redAccent.withOpacity(0.2) : surfaceDark,
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: _selectedOption == 0 ? Colors.redAccent : Colors.transparent,
                          width: 2,
                        ),
                      ),
                      child: const Column(
                        children: [
                          Icon(Icons.sentiment_dissatisfied, color: Colors.white54, size: 40),
                          SizedBox(height: 12),
                          Text('No, I regret it', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () => _submitReflection(1),
                    child: Container(
                      padding: const EdgeInsets.all(24),
                      decoration: BoxDecoration(
                        color: _selectedOption == 1 ? primaryColor.withOpacity(0.3) : primaryColor.withOpacity(0.1),
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: _selectedOption == 1 ? primaryColor : primaryColor.withOpacity(0.5),
                          width: 2,
                        ),
                      ),
                      child: const Column(
                        children: [
                          Icon(Icons.sentiment_satisfied, color: primaryColor, size: 40),
                          SizedBox(height: 12),
                          Text('Yes, worth it', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
