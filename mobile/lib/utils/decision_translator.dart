...
class DecisionTranslator {
  static DecisionExplanation translate(
    Map<String, dynamic> data, {
    String explanationStyle = 'concise',
  }) {
    // 1. Parse fields robustly
    final behaviorPenalty =
        (data['behavior_penalty'] as num?)?.toDouble() ?? 0.0;
    final affordabilityScore =
        (data['affordability_score'] as num?)?.toDouble() ?? 100.0;

    final breakdown = data['risk_breakdown'] as Map<String, dynamic>? ?? {};
    final behaveImpact = (breakdown['behavior'] as num?)?.toDouble() ?? 0.0;
    final affordImpact =
        (breakdown['affordability'] as num?)?.toDouble() ?? 0.0;
    ...
    if (deficitMode || liquidityFailure) {
      ...
      verdict =
          "Delay"; // High behavior, but affordable - tell them to sleep on it
    } else if (goalImpact > 30 || riskLevel.contains('Moderate')) {
      verdict = "Delay";
    }

    String dominantFactor = 'none';
    if (deficitMode || liquidityFailure) {
      dominantFactor = 'liquidity';
    } else if (behaveImpact > 0 &&
        behaveImpact >= affordImpact &&
        behaveImpact >= goalImpact) {
      dominantFactor = 'behavior';
    } else if (affordImpact > 0 && affordImpact >= goalImpact) {
      dominantFactor = 'affordability';
    } else if (goalImpact > 0) {
      dominantFactor = 'goal_impact';
    }

    final recommendedAction = verdict == 'Safe to buy'
        ? 'buy'
        : (verdict == 'Avoid' ? 'avoid' : 'delay');
